...
def register_handlers(app: App) -> None:
    ...
    @dp.message()
    async def fallback_support_prompt(message: Message, state: FSMContext) -> None:
        # Don't interrupt FSM flows (broadcast/greeting/etc.)
    ...
    async def cb_admin_broadcast(call: CallbackQuery, state: FSMContext) -> None:
        if not call.from_user or not await is_admin(call.from_user.id):
            await call.answer()
            return
        logging.info("Admin %s started broadcast FSM", call.from_user.id)
        await call.answer()
        await state.clear()
    ...
    @dp.message(StateFilter(BroadcastFSM.waiting_content))
    ...
    @dp.callback_query(StateFilter(BroadcastFSM.confirm), F.data == "broadcast:cancel")
    ...
    @dp.callback_query(StateFilter(BroadcastFSM.confirm), F.data == "broadcast:send")
    async def cb_broadcast_send(call: CallbackQuery, state: FSMContext) -> None:
        if not call.from_user or not await is_admin(call.from_user.id):
            await call.answer()
            return
        logging.info("Admin %s pressed broadcast send", call.from_user.id)
        data = await state.get_data()
        chat_id = data.get("chat_id") or data.get("from_chat_id")
        ...
        try:
            async with app.sessionmaker() as session:
                users = (
                    await session.execute(
                        select(User.user_id).where(User.is_blocked.is_(False)).order_by(User.created_at)
                    )
                ).scalars().all()
        except Exception as e:
            logging.exception("Failed to fetch users for broadcast: %s", e)
            await app.bot.send_message(call.from_user.id, "⚠️ Ошибка при чтении пользователей из БД. Смотрите логи.")
            await state.clear()
            return

        logging.info("Broadcast recipients: %s", len(users))
        await app.bot.send_message(
            call.from_user.id,
            f"⏳ Рассылка запущена. Получателей: <b>{len(users)}</b>",
            parse_mode=ParseMode.HTML,
        )

        if not users:
            await app.bot.send_message(call.from_user.id, "Нет пользователей для рассылки.")
            await state.clear()
            return
        ...
        for uid in users:
            try:
                await app.bot.forward_message(chat_id=uid, from_chat_id=chat_id, message_id=message_id)
                sent += 1
                ...
                wait_s = float(e.retry_after) + 0.5
                logging.warning("FloodWait for %s sec while sending to %s", wait_s, uid)
                ...
                try:
                    ...
                    failed += 1
                    logging.exception("Broadcast retry failed for %s: %s", uid, e2)
            except Exception as e:
                failed += 1
                logging.exception("Broadcast send failed for %s: %s", uid, e)
