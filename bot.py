...
def start_inline_kb():
    ...
    kb.button(text="🔗 БЕСПЛАТНАЯ РЕГИСТРАЦИЯ", url="https://vasilkov.digital/neuroclass")
    kb.button(text="✅ Я зарегистрировался / Продолжить", callback_data="registered")
    kb.adjust(1)
    return kb.as_markup()
...
async def check_membership(bot: Bot, user_id: int, channels: Iterable[str]) -> tuple[bool, list[str], bool]:
    """
    ...
    for ch in channels:
        try:
            ...
            logging.warning("getChatMember failed for %s: %s", ch, e)
            bot_not_admin = True
            missing.append(ch)
            continue
...
def register_handlers(app: App) -> None:
    ...
    async def cb_registered(call: CallbackQuery) -> None:
    ...
    async def cb_check_subscription(call: CallbackQuery) -> None:
        ...
        ok, missing, bot_not_admin = await check_membership(app.bot, user_id, [CHANNEL_1, CHANNEL_2])
